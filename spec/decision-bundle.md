...
- `tasks` and `assignments` are present together for independent replay, except that `assignments` may be empty for `DECISION_STATUS_INFEASIBLE`
...
- `signer_key_id` must identify the signing key or signer policy used to produce `signature`
...
- `DECISION_STATUS_PARTIAL`: the scheduler produced a degraded decision, such as partial placement, a fallback path, or a policy-approved relaxation. The bundle must record the reason in `fallback` and include any relevant failing or waived `constraint_evals`.
...
- verify the signature using the signer policy identified by `signer_key_id`
